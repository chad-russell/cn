...
- `hindsight.service` (rootless quadlet, crussell user units) — agent-memory server `ghcr.io/vectorize-io/hindsight:latest` (`hosts/bee/hindsight.nix`, live 2026-09-21): retain/recall/reflect API + per-bank MCP on Nebula `10.10.0.12:8888`, control-plane UI `:9999` (gated by `HINDSIGHT_CP_ACCESS_KEY`); full image runs BGE embeddings + MiniLM reranker locally, extraction LLM is `~z-ai/glm-flash-latest` via OpenRouter (key in agenix `hindsight-env.age`, crussell-readable for the user quadlet). Data: embedded pg0 Postgres in named volume `hindsight-data` (restic-covered via `~/.local/share/containers`; image layers excluded in `backup.nix`). Ingress: bees Caddy `hindsight.internal.crussell.io` (+ `-ui`) — wiki [tools/hindsight](https://git.crussell.io/chad/wiki) page has the runbook. Coding agents: `npx @vectorize-io/hindsight-coding-agents install <harness> --server self-hosted --api-url http://10.10.0.12:8888` (dsh wired 2026-09-21).
