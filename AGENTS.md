...
│   │   │   └── routes/internal/ (services.caddy, media.caddy, beszel.caddy, discuit.caddy)
│   │   ├── media-services.nix
...
│   │   ├── discuit.nix        # Discuit (discuit.internal.crussell.io) + .container + config yaml
...
| --------------- | ----------------- | ------------------------------------- | ----------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bees`          | `192.168.20.41`   | `10.10.0.6`                           | NixOS 26.05 | `hosts/bees/`                        | Production server: Caddy (**internal `*.internal.crussell.io` only**), ntfy, datenight, linkding, papra, Jellyfin, Sonarr, Radarr, Prowlarr, qBittorrent, Jellyseerr, Immich, Discuit. |
...
- `hosts/bees/discuit.nix` — Discuit at `discuit.internal.crussell.io` (quadlet, image built from pinned source; bootstrap + signups-closed runbook in the file header)
...
- `discuit.service` — Discuit quadlet, `127.0.0.1:8100` (app+MariaDB+Redis in one container; see `hosts/bees/discuit.nix`)
