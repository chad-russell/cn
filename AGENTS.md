...
**Agent identity on Forgejo:** agent sessions act as the `glen` Forgejo
user — never `chad` — so Chad's approvals/merges stay distinct from
agent-authored changes. Mechanics:

- API/PRs: `secrets/forgejo-token.age` (deployed as `FORGEJO_TOKEN`;
  scopes `write:repository,write:issue`). Rotate with
  `forgejo admin user generate-access-token -u glen -t <name> --scopes ...`
  on the gateway (as the `forgejo` user, `FORGEJO_WORK_DIR=/var/lib/forgejo`),
  then re-encrypt the agenix file.
- Pushes: SSH key **"glen forge SSH key (bee)"** (Proton Pass, Glen vault),
  registered on the glen account. Agent sandboxes can't write `~/.ssh`, so
  stage it per-call: extract the item's `SshKey.private_key` to a `0600`
  temp file and push with
  `GIT_SSH_COMMAND='ssh -F /dev/null -i <keyfile> -o IdentitiesOnly=yes'`.
- Commits: author as `Glen <glen@noreply.git.crussell.io>` (the account's
