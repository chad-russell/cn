...
- `hindsight.service` (rootless quadlet, crussell user units) — agent-memory server `ghcr.io/vectorize-io/hindsight:latest` (`hosts/bee/hindsight.nix`, live 2026-09-21): retain/recall/reflect API + per-bank MCP on Nebula `10.10.0.12:8888`, control-plane UI `:9999` (gated by `HINDSIGHT_CP_ACCESS_KEY`); full image runs BGE embeddings + MiniLM reranker locally, extraction LLM is `glm-5.3-flash` via the Z.AI coding-plan endpoint `https://api.z.ai/api/coding/paas/v4` (switched from OpenRouter per-token 2026-09-22 after a $7.05 first day; key = zai-api-key value in agenix `hindsight-env.age`, crussell-readable for the user quadlet). Data: embedded pg0 Postgres in named volume `hindsight-data` (restic-covered via `~/.local/share/containers`; image layers excluded in `backup.nix`). Ingress: bees Caddy `hindsight.internal.crussell.io` (+ `-ui`) — runbook is the `hindsight` skill (chad/skills). Coding agents: `npx @vectorize-io/hindsight-coding-agents install <harness> --server self-hosted --api-url http://10.10.0.12:8888` (dsh wired 2026-09-21).
