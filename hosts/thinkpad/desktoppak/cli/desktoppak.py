...
LAUNCH_SCRIPT = RUNTIME_DIR / "launch-bwrap-session.sh"
    ...
    if root.exists():
        ...
        if not manifest_ok:
            raise SystemExit(
                ...
                f"run: python3 desktoppak/cli/desktoppak.py uninstall --force {name}"
            )
...
def update(args: argparse.Namespace) -> None:
    ...
    staging_rootfs = install_dir / f"rootfs.new.{os.getpid()}"
    backup_rootfs = install_dir / f"rootfs.old.{os.getpid()}"

    if backup_rootfs.exists():
        shutil.rmtree(backup_rootfs, ignore_errors=True)

    try:
        extract_rootfs(ref, staging_rootfs)
        source_manifest = staging_rootfs / "usr/share/desktoppak/manifest.json"
        if not source_manifest.is_file():
            raise SystemExit(f"missing manifest in rootfs: {source_manifest}")
        manifest_text = source_manifest.read_text()
        if final_rootfs.exists():
            final_rootfs.rename(backup_rootfs)
        staging_rootfs.rename(final_rootfs)
        if backup_rootfs.exists():
            shutil.rmtree(backup_rootfs, ignore_errors=True)
        manifest_path(name).write_text(manifest_text)
        image_id_path(name).write_text(new_id + "\n")
    except Exception:
        if final_rootfs.exists() and backup_rootfs.exists():
            shutil.rmtree(final_rootfs, ignore_errors=True)
        if backup_rootfs.exists() and not final_rootfs.exists():
            backup_rootfs.rename(final_rootfs)
        raise
    finally:
        if staging_rootfs.exists():
            shutil.rmtree(staging_rootfs, ignore_errors=True)
...
def launch(args: argparse.Namespace) -> None:
    ...
    if not pkg_root(name).exists():
        raise SystemExit(f"not installed: {name}")
    env = os.environ.copy()
    env["DESKTOPPAK_ROOTFS_DIR"] = str(rootfs_dir(name))
    env["DESKTOPPAK_STATE_ROOT"] = str(runtime_state_root(name))
