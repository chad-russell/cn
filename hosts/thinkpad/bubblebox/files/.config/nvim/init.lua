...
--
-- Lives at ~/.config/nvim (the host dotfiles, reached via writable_home in the
-- nvim entrypoint). Plugin trees/state/cache are isolated to a bubblebox-owned
-- dir via XDG_DATA/STATE/CACHE_HOME -> /persist — see nvim/entrypoint.toml.
-- Edit this config live; no rebuild needed (changes land in the dotfiles repo
-- through the cjust link symlink).
