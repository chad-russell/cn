#!/usr/bin/env bash
#
# install.sh — link the gpl quadlet dev stack into the user systemd quadlet
...
"${SYS[@]}" list-unit-files 'gpl-dev*' 2>/dev/null || true

cat <<EOF

Done. Start the stack:
  ${SYS[*]} start gpl-dev-app
Stop it:
  ${SYS[*]} stop gpl-dev-app
Follow the app logs:
  ${SYS[*]} status gpl-dev-app   # or: journalctl --user -u gpl-dev-app -f

Browser: http://localhost:3006 (gpl)
EOF
