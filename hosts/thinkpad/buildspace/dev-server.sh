#!/usr/bin/env bash
#
# dev-server.sh — PID 1 of the buildspace-quadlet app container.
#
...
#   jobs :3010
#
# This file lives OUTSIDE the product repo
# (under ~/Code/cn/hosts/thinkpad/buildspace) and is bind-mounted read-only into
# the container at /usr/local/bin/dev-server.sh, so nothing is vendored into the
# buildspace checkout.
set -u
