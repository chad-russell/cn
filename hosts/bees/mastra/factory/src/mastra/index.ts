...
// cn delta (2026-09-23): the template skips credential encryption entirely
// when auth is disabled (auth === null). We run with MASTRACODE_AUTH_DISABLED=1
// on an auth-less internal deployment and still want provider/integration
// credentials encrypted at rest, so call credentialEncryption()
// unconditionally — it returns undefined when FACTORY_CREDENTIAL_ENCRYPTION_KEY
// is unset, matching template behavior for unconfigured deployments.
const secretEncryption = credentialEncryption();
