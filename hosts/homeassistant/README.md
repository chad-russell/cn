...
SSH username: `hassio`; password is configured in the add-on UI and should not be committed here.

Repo helpers:

```bash
# Open an SSH session
hosts/homeassistant/scripts/ha-ssh.sh

# Run Supervisor-backed HA CLI commands remotely
hosts/homeassistant/scripts/ha-supervisor.sh apps list --raw-json
```
...
```bash
hosts/homeassistant/scripts/ha-supervisor.sh apps list --raw-json | jq '.data.addons[] | {slug,state}'
```
...
```bash
hosts/homeassistant/scripts/ha-supervisor.sh apps info 45df7312_zigbee2mqtt --raw-json
hosts/homeassistant/scripts/ha-supervisor.sh apps logs 45df7312_zigbee2mqtt

hosts/homeassistant/scripts/ha-supervisor.sh apps info core_openthread_border_router --raw-json
hosts/homeassistant/scripts/ha-supervisor.sh apps logs core_openthread_border_router
```
...
```bash
hosts/homeassistant/scripts/ha-radio-audit.sh
```
...
- `hosts/homeassistant/incidents/2026-04-14-zigbee-thread-serial-port-collision.md`
- `hosts/homeassistant/scripts/ha-ssh.sh`
- `hosts/homeassistant/scripts/ha-supervisor.sh`
- `hosts/homeassistant/scripts/ha-radio-audit.sh`
