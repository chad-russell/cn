...
# start booting from `containers-storage:localhost/silverblue-thinkpad:44`
# (or to later re-point the host at a different image/tag). Uses the modern
# bootc front-end (not rpm-ostree).
#
#   bootc switch --transport containers-storage localhost/silverblue-thinkpad:44
#
...
After reboot, verify which build you're on:
  cat /usr/lib/silverblue-thinkpad-version
